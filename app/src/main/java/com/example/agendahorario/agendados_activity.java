        ...
        call.enqueue(new Callback<List<Agendamento>>() {
            ...
            public void onResponse(Call<List<Agendamento>> call, Response<List<Agendamento>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    // Filtra a lista para remover agendamentos com status "cancelled"
                    List<Agendamento> agendamentosAtivos = response.body().stream()
                            .filter(agendamento -> agendamento.getStatus() != null && !agendamento.getStatus().equalsIgnoreCase("cancelled"))
                            .collect(Collectors.toList());

                    agendamentos.clear();
                    agendamentos.addAll(agendamentosAtivos);
