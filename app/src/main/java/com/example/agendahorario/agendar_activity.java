...
public class agendar_activity extends AppCompatActivity {
    ...
    private void showCustomTimePickerDialog() {
        ...
        for (int hora = 8; hora <= 22; hora++) {
            horarios.add(String.format(Locale.getDefault(), "%02d:00", hora));
            if (hora < 22) {
                horarios.add(String.format(Locale.getDefault(), "%02d:30", hora));
            }
        }

        CharSequence[] items = horarios.toArray(new CharSequence[0]);

        android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(this);
        builder.setTitle("Selecione um Horário");
        builder.setItems(items, (dialog, which) -> {
            horarioSelecionado = horarios.get(which);
